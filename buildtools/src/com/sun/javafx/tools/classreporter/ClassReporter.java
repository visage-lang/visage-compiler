 ...
 * have any questions.
 */
package com.sun.javafx.tools.classreporter;
...
public class ClassReporter {
    ...
    static void CreateSampleFx(File fxFile) {
        ...
        doExec(getExe("javafxc").getAbsolutePath(), fxFile.getAbsolutePath());
    }
    ...
    static void parseOutput() {
        ...
        if (jarfilename == null) {
            File fxFile = new File(builddir, fxclassname + ".fx");
            CreateSampleFx(fxFile);
            output = doExec(getExe("javafx").getAbsolutePath(), "-cp",
                builddir, "-verbose:class", fxclassname  );
        } else {
            output = doExec(getExe("javafx").getAbsolutePath(),
