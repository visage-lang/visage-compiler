...
public class JavafxDecompose implements JavafxVisitor {
    ...
    private VisageVar makeVar(DiagnosticPosition diagPos, String label, VisageExpression initExpr, JavafxBindStatus bindStatus, Type type) {
        VisageVar var = preTrans.SynthVar(diagPos, currentVarSymbol, label, initExpr, bindStatus, type, inScriptLevel, varOwner);
        lbVar.append(var);
        return var;
    }

    private VisageVar makeVar(DiagnosticPosition diagPos, Name vName, VisageExpression pose, JavafxBindStatus bindStatus, Type type) {
    ...
    private VisageVar shredVar(String label, VisageExpression pose, Type type, JavafxBindStatus bindStatus) {
    ...
    private boolean isBoundFunctionResult(VisageExpression initExpr) {
        if (initExpr instanceof VisageFunctionInvocation) {
            Symbol meth = JavafxTreeInfo.symbol(((VisageFunctionInvocation)initExpr).meth);
    ...
    private VisageVar makeTempBoundResultName(Name varName, VisageExpression initExpr) {
        ...
        if (isBoundFunctionResult(initExpr)) {
                 ...
                 */
                ptrVar = makeVar(initExpr.pos(), tmpBoundResName, initExpr, JavafxBindStatus.UNIDIBIND, syms.visage_PointerType);
    ...
    public void visitIfExpression(VisageIfExpression tree) {
        ...
        if (bindStatus.isBound() && types.isSequence(tree.type)) {
            ...
            VisageVar v = makeSizeVar(tree.pos(), JavafxDefs.UNDEFINED_MARKER_INT);
            res.boundSizeVar = v;
        }
        result = res;
    }
    ...
    public void visitBreak(VisageBreak tree) {
        if (tree.nonLocalBreak) {
            ...
            VisageIdent nonLocalExceptionClass = fxmake.Ident(names.fromString(JavafxDefs.cNonLocalBreakException));
    ...
    public void visitContinue(VisageContinue tree) {
        if (tree.nonLocalContinue) {
            ...
            VisageIdent nonLocalExceptionClass = fxmake.Ident(names.fromString(JavafxDefs.cNonLocalContinueException));
    ...
    public void visitReturn(VisageReturn tree) {
        ...
        if (tree.nonLocalReturn) {
            ...
            VisageIdent nonLocalExceptionClass = fxmake.Ident(names.fromString(JavafxDefs.cNonLocalReturnException));
    ...
    public void visitFunctionInvocation(VisageFunctionInvocation tree) {
        ...
        if (bindStatus.isBound() && types.isSequence(tree.type) && !isBoundFunctionResult(tree)) {
            ...
            VisageVar sz = makeSizeVar(v.pos(), JavafxDefs.UNDEFINED_MARKER_INT);
    ...
    public void visitUnary(VisageUnary tree) {
        JavafxTag tag = tree.getFXTag();
        VisageExpression arg = tag == JavafxTag.REVERSE ||
   ...
   public void visitInstanciate(VisageInstanciate tree) {
    ...
    private VisageVar makeIntVar(DiagnosticPosition diagPos, String label, int initial) {
        ...
        VisageVar v = makeVar(diagPos, label, initialSize, JavafxBindStatus.UNBOUND, syms.intType);
        return v;
    }

    private VisageVar makeSaveVar(DiagnosticPosition diagPos, Type type) {
        VisageVar v = makeVar(diagPos, defs.saveNamePrefix(), null, JavafxBindStatus.UNBOUND, type);
        v.sym.flags_field |= JavafxFlags.VARMARK_BARE_SYNTH;
        return v;
    }
    ...
    public void visitForExpression(VisageForExpression tree) {
        if (bindStatus.isBound()) {
            ...
            VisageVar helper = makeVar(tree, helperName, init, JavafxBindStatus.UNBOUND, helperType);
