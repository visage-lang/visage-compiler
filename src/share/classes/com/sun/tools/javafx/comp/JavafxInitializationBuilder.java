            ...
            if (!hasFxSuper) {
                ...
                for (JCTree member : cDefinitions) {
                    if (member.getTag() == JCTree.METHODDEF) {
                        ...
                        excludes.add(jcmeth.name.toString());
                    }
                }
   ...
   private JCMethodDecl makeConstructor(DiagnosticPosition diagPos, List<JCVariableDecl> params, List<JCStatement> cStats) {
    ...
    class JavaCodeMaker {
        ...
        JCExpression makeFlagExpression(VarInfo varInfo, String action, String flag) {
            ...
            Name name = names.fromString(action + flag);
            // Construct the arg.
            JCExpression arg = Id(attributeOffsetName(varInfo.getSymbol()));
            // Return expression.
            return m().Apply(null, Id(name), List.<JCExpression>of(arg));
            }

        //
        // This method returns a statement for setting/clearing a var flag.
        //
        JCStatement makeFlagStatement(VarInfo varInfo, String action, String flag) {
            return m().Exec(makeFlagExpression(varInfo, action, flag));
        }
        ...
        private List<JCTree> cloneFXBaseVar(VarSymbol var, HashSet<String> excludes) {
            ...
            members.append(makeMethod(flags, type, name, List.<JCVariableDecl>nil(), stmts));
            // Add to the exclusion set.
            excludes.add(name.toString());
            ...
            members.append(makeMethod(flags, syms.voidType, name, List.<JCVariableDecl>of(arg), stmts));
            // Add to the exclusion set.
            excludes.add(name.toString());
            ...
            for (VarInfo ai : attrInfos) {
                ...
                if (ai.needsDeclaration()) {
                    ...
                    if (deflt != null) {
                        ...
                        if (!ai.isDef()) {
                            ...
                            if (enumeration >= 0) {
                                ...
                                // if (!isInitialized(VOFF$var)) == 0) { applyDefaults$var(); }
                                deflt = m().If(NOT(makeFlagExpression(ai, varFlagActionTest, varFlagInitialized)), deflt, null);
                            }
                        }
                    ...
                    if (init != null) {
                        ...
                        JCExpression isNotInitializedExpr = NOT(isInitializedExpr);
                        // varNum == VOFF$var
