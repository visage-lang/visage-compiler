...
public class Scanner implements Lexer {
    ...
    private static class BraceQuoteTracker {
        ...
        static void enterQuoteBrace(char quote, boolean percentIsFormat) {
            quoteStack = new BraceQuoteTracker(quoteStack, quote, percentIsFormat); // push
        }
        static void enterBrace(boolean percentIsFormat) {
            if (quoteStack != null) {
        ...
        static boolean checkAndResetPercentIsFormat() {
            boolean isFormat = quoteStack.percentIsFormat;
     ...
     */
    private boolean isSpecial(char ch) {  // TODO JavaFX: check these
        switch (ch) {
        case '%': case '&': case '*': case '?':
    ...
    public void nextToken() {
        ...
        try {
            ...
            while (true) {
                ...
                case '{':
                    scanChar();
                    BraceQuoteTracker.enterBrace(false);
                    token = LBRACE;
                    return;
                case '}': {
                    scanChar();
                    char quote = BraceQuoteTracker.leaveBrace();
                    if (quote != 0) {
                        while (ch != quote && ch != '{' && bp < buflen)
                            scanLitChar();
                        if (ch == quote) {
                            token = RBRACE_QUOTE_STRINGLITERAL;
                            scanChar();
                            BraceQuoteTracker.leaveQuote();
                        } else if (ch == '{') {
                            scanChar();
                            skipWhite();
                            BraceQuoteTracker.enterBrace(ch == '%');
                            token = RBRACE_LBRACE_STRINGLITERAL;
                        } else {
                ...
                case '\"':
                    ...
                    if (ch == '\"') {
                        ...
                        scanChar();
                        skipWhite();
                        BraceQuoteTracker.enterQuoteBrace('\"', ch == '%');
                        token = QUOTE_LBRACE_STRINGLITERAL;
                    } else {
                        lexError(pos, "unclosed.str.lit");
                    }
                    return;
                ...
                case '<':
                    ...
                    if (ch == '<') {
                        ...
                        do {
                            ...
                            if (closeBraceCount < 2) {
                                // this isn't the end, restore the '>' we ate
                                putChar('>');
                            }
                        } while (closeBraceCount < 2);
                        // add back any non-terminating braces
                        for (int i = 2; i < closeBraceCount; ++i) {
                            putChar('>');
                        }
                        name = names.fromChars(sbuf, 0, sp);
                        token = IDENTIFIER;
                    ...
                    return;
                case '%':
                    if (BraceQuoteTracker.checkAndResetPercentIsFormat()) {
                        scanLitChar();
                        // spaces can occur before the conversion character.
                        // conversion character is the first alpha char.
                        // so blindly include everything upto an alpha.
                        while (!((ch >= 'A' && ch <='Z') || (ch >= 'a' && ch <='z')) && bp < buflen) {
                            scanLitChar();
                        }
                        // now look for the terminating white
                        while (ch != ' ' && ch != '\t' && ch != FF && ch != LF && ch != CR && bp < buflen) {
                            scanLitChar();
                        }
                        token = FORMAT_STRINGLITERAL;
                        return;
                    }
                default:
                    if (isSpecial(ch)) {
