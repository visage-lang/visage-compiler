...
objectLiteralPart  returns [JFXStatement value]
	: name COLON  bindOpt expression (COMMA | SEMI)?	{ $value = F.at(pos($COLON)).ObjectLiteralPart($name.value, $expression.expr, $bindOpt.status); }
       ...
       | variableDefinition ;
bracketExpression : LBRACKET   (   generatorClause   |   dotDotClause   |   expressionListOpt   )   RBRACKET ;
dotDotClause : expression   (   COMMA   expression   ) ?   DOTDOT   LT ?   expression ;
generatorClause : expression   BAR   generator   (     COMMA   (   generator   |   expression   )     ) * ;
