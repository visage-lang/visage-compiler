 ...
 * @author Brian Goetz
 */
public class LegacySequenceVar<T> extends AbstractSequenceLocation<T> implements SequenceLocation<T>, MutableLocation {
    ...
    public static <T> SequenceLocation<T> make(Sequence<T> value) {
        return new LegacySequenceVar<T>(value);
    }

    public static <T> SequenceLocation<T> make(Class<T> clazz, Sequence<? extends T> value) {
        return new LegacySequenceVar<T>(clazz, value);
    }

    public static <T> SequenceLocation<T> makeUnmodifiable(Sequence<T> value) {
        return Locations.unmodifiableLocation(new LegacySequenceVar<T>(value));
    }

    private LegacySequenceVar(Sequence<T> value) {
        super(value.getElementType(), true);
        replaceValue(value);
    }

    private LegacySequenceVar(Class<T> clazz, Sequence<? extends T> value) {
        super(clazz, true);
        if (value == null)
    ...
    private class MutationListener implements SequenceMutator.Listener<T> {
        public void onReplaceSlice(int startPos, int endPos, Sequence<? extends T> newElements, Sequence<T> oldValue, Sequence<T> newValue) {
            LegacySequenceVar.this.replaceSlice(startPos, endPos, newElements, newValue);
        }
    }
}
