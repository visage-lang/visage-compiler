...
/**
 *  This class could be the main entry point for Visagedoc when Visagedoc is 
 ...
 *  @author Tom Ball, Visagedoc port
 */
public class VisagedocTool extends org.visage.tools.main.VisageCompiler {
    DocEnv docenv;

    final Context ctx;
    final Messager messager;
    final VisageClassReader clsreader;
    final VisagedocEnter jdenter;
     ...
     */
    protected VisagedocTool(Context context) {
        super(context);
        this.ctx = context;
        messager = Messager.instance0(context);
        clsreader = VisageClassReader.instance(context);
        jdenter = VisagedocEnter.instance0(context);
     ...
     */
    public static VisagedocTool make0(Context context) {
        try {
            // Because of circularities we need to register these services
            // before we allocate VisageClassReader, which needs to be done
            // before we allocate VisagedocClassReader, which needs to be
            // done before we allocate VisagedocTool.  Hence we do all this
            // stuff here rather than in registerServices.  Sigh.
            VisageFileManager.preRegister(context);
            VisagedocEnter.preRegister(context);
            VisagedocMemberEnter.preRegister(context);
            VisageSymtab.preRegister(context);
            JavadocTodo.preRegister(context);
            VisageTypes.preRegister(context);
            DocCommentScanner.Factory.preRegister(context);

            VisageClassReader reader = VisageClassReader.instance(context);
            VisagedocClassReader jclsreader = new VisagedocClassReader(context);
            reader.jreader = jclsreader;

            return new VisagedocTool(context);
