...
public class FXThreadReference extends FXObjectReference implements ThreadReference {
    ...
    private List<StackFrame> getFilteredFrames() throws IncompatibleThreadStateException {
        ...
        try {
            for (StackFrame fr : frames) {
                ...
                if (fxfr.isJavaFXFrame() && isInternal(fxfr.location().method())) {
                    continue;
                } else {
    ...
    private boolean isInternal(FXMethod method) {
        // FIXME: is there better way to detect internal (compiler generated) method?
        // Any method with "$" in it's name and has no LineNumberTable is treated
        // as "internal" method generated by compiler.
        String methodName = method.name();
        if (methodName.indexOf('$') != -1) {
            // exempt bound functions.
            if (methodName.contains("$bFunc$")) {
                return false;
            }
            try {
