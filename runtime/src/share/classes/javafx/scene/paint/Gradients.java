...
class Gradients {
    ...
    private static class Factory5 implements Factory {
        private com.sun.javafx.scene.paint.MultipleGradientPaint.CycleMethod cm(int cycleMethodInt) {
            if (cycleMethodInt == 1) {
                return com.sun.javafx.scene.paint.MultipleGradientPaint.CycleMethod.REFLECT;
            }
            else if (cycleMethodInt == 2) {
                return com.sun.javafx.scene.paint.MultipleGradientPaint.CycleMethod.REPEAT;
            }
            else {
                return com.sun.javafx.scene.paint.MultipleGradientPaint.CycleMethod.NO_CYCLE;
            }
        }

        public Paint createLinearGradientPaint(Point2D start, Point2D end, float[] fractions, Color[] colors, int cycleMethodInt) {
            return new com.sun.javafx.scene.paint.LinearGradientPaint(start, end, fractions, colors, cm(cycleMethodInt));
        }

        public Paint createRadialGradientPaint(Point2D center, float radius, Point2D focus, float[] fractions, Color[] colors, int cycleMethodInt) {
            return new com.sun.javafx.scene.paint.RadialGradientPaint(center, radius, focus, fractions, colors, cm(cycleMethodInt));
        }
    }
