...
int main(int argc, char** argv) {
    ...
    if(config.profile_emulator.empty()) {
        ...
        cmd = "\"" + util.evaluatePath(config.visagepath, config.profile_emulator) + "\" ";
    }
    ...
    if (! config.profile_nativelibpath.empty()) {
        if (config.librarypath.empty()) {
            ...
            cmd += "-Djava.library.path=\"" + config.librarypath + ";" + util.evaluatePath(config.visagepath, config.profile_nativelibpath) + "\" ";
        }
    } else if (! config.librarypath.empty()) {
        cmd += "-Djava.library.path=\"" + config.librarypath  + "\" ";
    }
    if (! config.profile_bootnativelibpath.empty()) {
        cmd += "-Dsun.boot.library.path=\"" + util.evaluatePath(config.visagepath, config.profile_bootnativelibpath + "\" ");
    }
    if (! config.securitypolicy.empty()) {
        ...
        cmd += "-Djava.security.policy=\"" + util.evaluatePath(config.visagepath, config.profile_security_policy + "\" ");
    }
