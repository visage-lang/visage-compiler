...
public class Canvas extends Component {
    ...
    private function updateCachedBounds(node:Node):Void {
        node.cachedBounds = node.getFXNode().getBounds();
        node.cachedXYWH = node.getSGNode().getBounds();
    }
    ...
    private function doLayout(root:Node):Void {
        if (root.needsLayout) {
            if (root instanceof Group) {
                ...
                doLayout((root as CustomNode).content);
            }
        }
        if (root instanceof Group) {
    ...
    public attribute content: Node[] on replace oldNodes[a..b] = newNodes {
        for (node in oldNodes[a..b]) {
            getRoot().remove(node.getFXNode());
        }
        var index = a;
        for (node in newNodes) {
            getRoot().add(index, node.getFXNode());
            index = index + 1;
        }
    }
