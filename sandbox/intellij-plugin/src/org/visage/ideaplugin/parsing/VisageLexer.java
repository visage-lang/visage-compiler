 ...
 * @author Brian Goetz
 */
public class VisageLexer extends LexerBase {
    ...
    public VisageLexer(boolean ignoreSyntheticSemi) {
    ...
    public IElementType getTokenType() {
        ...
        if (tokenType == v3Lexer.EOF)
            return null;
        IElementType result = VisageTokens.getElement(tokenType);
        if (result == null) {
...
//        System.out.printf("Processed %d:%s @ %d:%d/%d => %d%n", nextToken.getType(), VisageTokens.getElement(nextToken.getType()), getTokenStart(), getTokenEnd(), bufferEnd, nextState);
        if (curEnd == curStart && nextToken.getType() != v3Lexer.EOF)
            System.out.printf("Failed to advance position: %d:%d/%d(%s:%s)%n", curStart, curEnd, bufferEnd, VisageTokens.getElement(nextToken.getType()), nextToken.getText());
    }
