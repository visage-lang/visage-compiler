...
public enum VisageTokens {
    ...
    static {
        int max = 0;
        for (VisageTokens t : VisageTokens.values())
    ...
    VisageTokens(int value) {
        tokenValue = value;
        elementType = new VisageElementType(name(), VisagePlugin.VISAGE_LANGUAGE, value);
    }
    ...
    public static TokenSet createTokenSet(VisageTokens... tokens) {
    ...
    private static void checkForMissingOrInvalidTokens () {
        ...
        for (Field field : fields) {
            ...
            if (value < 0  ||  value >= tokenArray.length)
                ...
                VisageElementType type = tokenArray[value];
                if (type == null)
                    error = true;
                else
                    if (type.antlrToken != value)
                        error = true;
                VisageTokens token = null;
                try {
                    token = valueOf (name);
            ...
            if (error)
                System.out.println ("ERROR: VisageTokens class has to be updated: additional or incompatible token in v3Lexer class: Name/Value: " + name + "/" + value);
        }
    }
