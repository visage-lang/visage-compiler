...
public class FxCompiler implements TranslatingCompiler {
    ...
    public ExitStatus compile(final CompileContext compileContext, VirtualFile[] virtualFiles) {
        ...
        for (Map.Entry<Module, Set<VirtualFile>> entry : map.entrySet()) {
            ...
            ClassLoader loader;
            try {
                loader = new URLClassLoader (new URL[] { new URL ("file://" + jdk.getHomeDirectory ().getPath () + "/lib/visagec.jar") }, getClass ().getClassLoader ());
            } catch (MalformedURLException e) {
                compileContext.addMessage (CompilerMessageCategory.ERROR, "Cannot locate visagec.jar library in SDK: " + jdk.getName (), null, 0, 0);
                compilationFailed.set (true);
                break;
            }
            ...
            Class<?> visageTaskClass;
            Method generateMethod;
            Method toUriMethod;
            try {
				Class<?> visageToolClass = Class.forName("com.sun.tools.visage.api.JavafxcTool", true, loader);
				...
				Class<?> visageFileManagerClass = Class.forName("com.sun.tools.visage.util.JavafxFileManager", true, loader);
                ...
                visageTaskClass = Class.forName ("com.sun.visage.api.JavafxcTask", true, loader);
            ...
            for (VirtualFile file : files) {
                ...
                try {
                    Object javacFileManager = getStandardFileManagerMethod.invoke (visageTool, null, null, Charset.defaultCharset ());
            ...
            Object compilerTask;
            try {
                compilerTask = getTaskMethod.invoke (visageTool, null, null, new DiagnosticListener() {
    ...
    private VirtualFile findSourceFileFor (Set<VirtualFile> sources, ModuleRootManager rootManager, String destRelative) {
        ...
        for (VirtualFile source : sources) {
            ...
            if (! sourceRelative.endsWith (".visage"))
                continue;
